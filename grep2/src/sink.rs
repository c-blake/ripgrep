...
/// implement this trait explicitly. It's like that using `io::Error` (which
...
/// implementation themselves, this crate chooses the "push" or "internal"
/// model of execution. Thus, in order to act on search results, callers must
/// provide an implementation of this trait to a searcher, and the searcher
/// is then responsible for calling the methods on this trait.
///
...
///   [`SinkError`](trait.SinkError.html) trait.
...
/// * What to do when a search has finished successfully.
///
...
pub trait Sink {
    ...
    ///
    /// Errors of this type are not only built by the methods on this trait,
    /// but the constructors defined in `SinkError` are also used in the
    /// searcher implementations themselves. e.g., When a I/O error occurs when
    /// reading data from a file.
    type Error: SinkError;

    /// This method is called whenever a match is found.
    ///
    /// If multi line is enabled, then match reported here may span multiple
    /// lines and it may include multiple matches. When multi line is disabled,
    /// then the match is guaranteed to span exactly one non-empty line (where
    /// a single line is, at minimum, a line terminator).
    ///
    ...
    where F: FnMut(u64, &str) -> Result<bool, io::Error>
    {
              ...
              M::Error: fmt::Display
        {
            ...
            let matched = match str::from_utf8(mat.bytes()) {
                ...
                // allocation here, but `std` doesn't provide such an
                // API. Regardless, this only happens on matches invalid
                // UTF-8, which should be pretty rare.
